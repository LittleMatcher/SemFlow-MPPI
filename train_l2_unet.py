"""
Training Script for L2 Layer with Unet Architecture

This script trains the L2 Safety-Embedded CFM layer using Unet architecture
with synthetic cost maps and trajectories generated by MockL3Dataset or
from generated data files.

Usage:
    # Train with mock data
    python train_l2_unet.py --num_samples 5000 --epochs 50
    
    # Train with generated data
    python train_l2_unet.py --data_source generated --data_dir traj_data/cfm_env --epochs 500 --batch_size 64
    
    # Train with specific Unet configuration
    python train_l2_unet.py --data_source generated --data_dir traj_data/cfm_env \
        --unet_base_channels 128 --unet_channel_mults 1 2 4 8 --epochs 500
...
If Train/Val Loss hardly drop (especially Acc >> Pos,Vel):
  ...
  - Batches: use smaller --batch_size (e.g. 64) so more gradient updates per epoch.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train L2 layer with Unet architecture")
    ...
    parser.add_argument('--hidden_dim', type=int, default=256,
                        help='Hidden dimension (for condition encoding)')
    parser.add_argument('--unet_base_channels', type=int, default=128,
                        help='Unet base channels')
    parser.add_argument('--unet_channel_mults', type=int, nargs='+', default=[1, 2, 4, 8],
                        help='Unet channel multipliers (e.g., 1 2 4 8)')
    ...
    parser.add_argument('--save_dir', type=str, default='checkpoints_l2_unet',
...
def create_model(args):
    """Create L2 model with Unet architecture."""
    model = create_l2_safety_cfm(
        model_type='unet1d',  # Always use Unet
        state_dim=2,
        max_seq_len=args.seq_len,
        hidden_dim=args.hidden_dim,
        num_layers=8,  # Not used for Unet, but kept for compatibility
        num_heads=8,   # Not used for Unet, but kept for compatibility
        cost_map_channels=1,
...
def save_checkpoint(model, optimizer, epoch, losses, save_path):
    ...
    checkpoint = {
        ...
        'losses': losses,
        'model_type': 'unet1d',  # Mark as Unet model
    }
...
def main():
    ...
    print("=" * 80)
    print("L2 Layer Training with Unet Architecture")
    print("=" * 80)
    ...
    print(f"  Res blocks per level: {args.unet_num_res_blocks}")
