...
class MPPI_BSpline:
    ...
    def __init__(self,
        ...
        Args:
            ...
            random_sample_ratio: 每次采样中“全局随机控制点”的比例（0-1）。
                >0 会显著增加探索多种同伦类路径的能力（随机重启），
                代价是会产生更多不可行样本（碰撞/越界）并增加方差。
            random_noise_std: 全局随机样本的噪声标准差；None 表示使用 noise_std。
        """
        ...
        self.random_sample_ratio = float(np.clip(random_sample_ratio, 0.0, 1.0))
        self.random_noise_std = float(noise_std if random_noise_std is None else random_noise_std)
    ...
    def _evaluate_trajectory_batch(args):
        ...
        # 批量计算
        return [bspline.evaluate(cp, n_samples=n_timesteps) for cp in control_points_batch]
    ...
    def sample_trajectories(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        """
        # 基础采样：围绕当前控制点局部扰动
        base_control_points = np.tile(
            self.control_points[np.newaxis, :, :],
            (self.n_samples, 1, 1)
        )

        # 可选：混入一部分“全局随机控制点”样本（随机重启），增加探索多种可能路径
        n_random = int(round(self.n_samples * self.random_sample_ratio))
        n_random = max(0, min(n_random, self.n_samples))
        if n_random > 0:
            random_cps = self.bspline.sample_random_control_points(
                start=self.start,
                goal=self.goal,
                bounds=self.bounds,
                n_samples=n_random
            )
            base_control_points[:n_random, :, :] = random_cps

        # 向控制点添加噪声（除了起始和目标），支持“局部样本”和“全局样本”不同噪声强度
        noise = np.random.randn(*base_control_points.shape)
        noise[:, 0, :] = 0.0
        noise[:, -1, :] = 0.0
        stds = np.full((self.n_samples, 1, 1), float(self.noise_std), dtype=float)
        if n_random > 0:
            stds[:n_random, :, :] = float(self.random_noise_std)
        sampled_control_points = base_control_points + noise * stds
